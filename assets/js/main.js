...
const ClockIn = {
    ...
    init: function() {
        // Prevent double-initialization (template also calls init)
    ...
    bindEvents: function() {
        var self = this;
        // Use namespaced events to avoid duplicates
        $(document).off('click.clockin', '#submitClockIn');
        $(document).on('click.clockin', '#submitClockIn', function() { self.submit(); });
        $(document).off('keypress.clockin', '#clockInPassword');
        $(document).on('keypress.clockin', '#clockInPassword', function(e) {
            if (e.which === 13) self.submit();
        });
        $(document).off('click.clockin', '#togglePassword');
        $(document).on('click.clockin', '#togglePassword', function() {
            var input = $('#clockInPassword');
    ...
    checkStatus: function() {
        Stand120.ajax('get_clock_in_status').then(function(response) {
            if (response.success && response.data.clocked_in) {
                ...
                $('#clockInStatus').hide();
                $('#clockInForm').show();
                // Delay focus slightly so mobile keyboards open reliably
                setTimeout(function() { $('#clockInPassword').trigger('focus'); }, 300);
            }
        }).catch(function() {
    ...
    submit: function() {
        ...
        Stand120.ajax('submit_clock_in', {
            ...
            if (response.success) {
                ...
                Stand120.showAlert('danger', response.data && response.data.message ? response.data.message : 'Clock-in failed');
            }
        }).catch(function() {
            $('#submitClockIn').prop('disabled', false).html('<i class="fas fa-check-circle"></i> Clock In Now');
...
const ClockInHistory = {
    ...
    bindEvents: function() {
        var self = this;
        $(document).off('click.clockhist', '#prevMonthClockIn');
        $(document).on('click.clockhist', '#prevMonthClockIn', function() {
            self.currentMonth--;
        ...
        $(document).off('click.clockhist', '#nextMonthClockIn');
        $(document).on('click.clockhist', '#nextMonthClockIn', function() {
            self.currentMonth++;
        ...
        // Keep fallback on generic #prevMonth/#nextMonth for existing templates
        $(document).off('click.clockhist', '#prevMonth');
        $(document).on('click.clockhist', '#prevMonth', function() {
            // Only handle if we're on the clock-in-history page
            if (!$('#clockInHistoryContent').length) return;
            self.currentMonth--;
            if (self.currentMonth < 1) {
                self.currentMonth = 12;
                self.currentYear--;
        ...
        $(document).off('click.clockhist', '#nextMonth');
        $(document).on('click.clockhist', '#nextMonth', function() {
            if (!$('#clockInHistoryContent').length) return;
            self.currentMonth++;
            if (self.currentMonth > 12) {
                self.currentMonth = 1;
        ...
        $(document).off('click.clockhist', '#exportCsv');
        $(document).on('click.clockhist', '#exportCsv', function() { self.exportCsv(); });
        $(document).off('click.clockhist', '#saveDeviceSettings');
        $(document).on('click.clockhist', '#saveDeviceSettings', function() { self.saveSettings(); });
        $(document).off('click.clockhist', '#useCurrentIp');
        $(document).on('click.clockhist', '#useCurrentIp', function() {
            var currentIp = $('#currentDetectedIp').text().trim();
